...
export default class index extends Component {
    ...
    UNSAFE_componentWillMount() {
        ...
        axios.get('http://localhost:3000/api1/information/collection?userid=' + isAuth.id).then(
            response => {
    ...
    deletePosts = (e) => {
        ...
        const aa = []
        const isAuth = JSON.parse(localStorage.getItem("phone"))
        axios.get('http://localhost:3000/api1/information/collection?userid=' + isAuth.id + "&&postsid=" + id).then(
            response => {
                const res = response.data
                let Posts_arr = new Array(res.length).fill('').map((item, index) => index);
                Posts_arr.map((item) => {
